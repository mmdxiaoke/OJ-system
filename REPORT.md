...
**代码规模**：后端 3 328 行（37 个文件）、前端 1 611 行（10 个文件）、测试 1 101 行（4 个文件），合计约 6 000 行 Python
...
2. **真实运行**生成脚本得到输入、用标程产出期望输出、在小数据上与暴力解交叉验证 →
3. 输出可入库的完整配置。
...
| 前端渲染与交互测试 | `python run.py test-frontend` | **31 / 31 通过**        |
| 代码规范     | `python -m ruff check .`      | **All checks passed** |

端到端测试会自行拉起真实 uvicorn 服务（临时数据目录 + 随机端口）和本地假模型服务，
不依赖任何外部网络。前端测试使用 Streamlit 官方 `AppTest` 真正执行页面脚本，
并模拟「登录 → 用表单建题 → 界面提交并轮询 → 下拉查看详情 → 改用户角色」的完整操作链。
...
前端共 5 个导航页，均通过 REST API 与后端交互：

| 页面      | 主要内容                                            |
| ------- | ----------------------------------------------- |
| 仪表盘     | 题库题数、我的提交/通过数、最近 5 条提交、快捷操作指引；管理员另见用户数与最近日志访问    |
| 用户中心    | 登录/注册/退出、个人信息与统计、最近提交、管理员用户列表与角色变更、创建管理员、后端地址设置  |
| 题库      | 题目列表、详情（含样例与完整配置、可下载 JSON）、图形化新增/编辑、删除、日志可见性配置    |
| 评测中心    | 代码提交（含代码模板与自动轮询）、提交记录筛选分页、提交详情与测试点明细、语言注册、重新评测   |
| AI 智能命题 | 模型配置、命题任务创建、SSE 实时进度与进度条、任务列表与中断、Token 用量与费用、一键导入题库 |

为了让「不查文档也能用」，前端做了这些易用性设计：

* **不手输 ID**：题目、语言、提交记录、用户全部改成下拉选择，标签显示为 `id · 标题`、
  `#提交号 · 题目 · 状态 · 得分` 等可读形式；
* **图形化题目编辑器**：字段逐项填写，样例与测试点用「选数量 + 逐条填写输入输出」的方式编辑，
  同时保留 JSON 高级模式；ID 已存在时自动切换为覆盖保存；
* **代码模板与自动轮询**：按当前语言一键插入代码骨架，提交后进度条实时更新并内联展示结果；
* **提交记录直接展开详情**：无需跳页，选中某条记录即可看到编译信息、运行结果与逐测试点明细；
* **缓存与刷新**：列表数据带会话内缓存，页面提供 🔄 按钮、侧边栏提供「刷新」一键清缓存。
...
| 质量保证           | 228 项接口断言 + 31 项前端断言 + ruff 零告警；所有 AI 生成的关键代码都由人工逐段复核                                              |
